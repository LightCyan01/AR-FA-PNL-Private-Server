...
fn direct_statuses_stack_tick_and_gate_the_turn() {
    ...
    for (effect_id, skill_id, duration) in [(780063000, 32002681, 2), (780056012, 32000809, 3)] {
    ...
    let shared_poison = rule_for(1039, "active", "skill", 20000627)
        ...
        .unwrap();
    assert_eq!(
        (shared_poison.state_id, shared_poison.duration),
        (940007, 1)
    );
    let poison_override = rule_for(1039, "active", "skill", 20008490)
        ...
        .unwrap();
    assert_eq!(
        (poison_override.state_id, poison_override.duration),
        (940007, 3)
    );
