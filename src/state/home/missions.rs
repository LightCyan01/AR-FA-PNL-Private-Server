...
pub(crate) fn objective_count(
    ...
    match objective {
        ...
        ResourceObjective::Characters => message_list(resources, "characters").len() as i32,
        ResourceObjective::Recipes => message_list(resources, "recipes").len() as i32,
        ...
        | ResourceObjective::MemoriaLevel { minimum, .. } => {
            ...
            let values: Vec<_> =
                message_list(resources, if character { "characters" } else { "memorias" })
                    .iter()
        ...
        ResourceObjective::Board { field, minimum } => message_list(resources, "characters")
            .iter()
        ...
        ResourceObjective::Equipment {
            ...
            let Some(character) = message_list(resources, "characters")
                .into_iter()
                    ...
                    message_list(resources, "equipment_tools")
                        ...
                        .any(|tool| {
                            entity.is_some()
                                ...
                                && i32_field(tool, "tool_id")
                                    .is_some_and(|id| tool_ids.contains(&id))
                                && message_list(tool, "traits").iter().any(|t| {
        ...
        ResourceObjective::EquippedMemoria {
            ...
            let entity = message_list(resources, "characters")
                .iter()
                .find(|r| i32_field(r, "character_id") == Some(*character_id))
                .and_then(|r| message_i32_field(r, "memoria_entity_id", "value"));
            i32::from(
                entity.is_some()
                    && message_list(resources, "memorias").iter().any(|r| {
        ...
        ResourceObjective::MemoriaLimit { memoria_id } => message_list(resources, "memorias")
            .iter()
            .filter(|row| i32_field(row, "memoria_id") == Some(*memoria_id))
            .filter_map(|row| i32_field(row, "limit_break"))
            .max()
            .unwrap_or(0),
        ResourceObjective::Research { group_id } => message_list(resources, "research_groups")
            .iter()
        ...
        ResourceObjective::Communication { character_ids } => {
            message_list(resources, "communication_states")
                .iter()
                .filter(|row| {
                    i32_field(row, "character_id").is_some_and(|id| character_ids.contains(&id))
                })
        ...
        ResourceObjective::NeoOpen => message_list(resources, "characters")
            .iter()
            .filter(|row| {
        ...
        ResourceObjective::Housing { house_building_id } => {
            message_list(resources, "house_building_states")
                .iter()
        ...
        ResourceObjective::PresentCloseness { present_ids } => {
            message_list(resources, "present_states")
                .iter()
                .filter(|row| {
        ...
        ResourceObjective::StreetPhase { quest_id } => message_list(resources, "street_states")
            .iter()
        ...
        ResourceObjective::OwnedHomeMotion { motion_ids } => {
            message_list(resources, "chara_home_motions")
                .iter()
                .filter(|row| {
        ...
        ResourceObjective::MultiMission { multi_mission_id } => {
            let count = message_list(resources, "multi_missions")
                .iter()
        ...
        ResourceObjective::ShipLevel => message_list(resources, "ships")
            .iter()
            .flat_map(|ship| {
                rules
                    .ship_levels
        ...
        ResourceObjective::ShipParty => {
            i32::from(message_list(resources, "ship_parties").iter().any(|row| {
...
pub(crate) fn mission_count(resources: &DynamicMessage, rule: &MissionRule) -> i32 {
    rule.total_task_condition_id
        ...
        .unwrap_or_else(|| {
            message_list(resources, "missions")
                .iter()
